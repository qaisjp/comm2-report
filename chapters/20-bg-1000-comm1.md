...
All pages on the website come from the `index.php` script and, in terms of visual design,
follow a consistent template:

- country flags to change translation
    - these country flags, when clicked,
- a sidebar
...
Individual with specific pages indicated by GET parameters in the query string, such as `/index.php?p=stats&show=vehicles&id=254572`.
...
If the `oset` query parameter is `0` (or missing, in the initial case), no pagination buttons are shown and only a
single "MORE.." button is shown. The `oset` parameter is not capped, and can go as far as the earliest resource that fits
the above criteria.
